...
function CreateServerPage() {
  ...
  const createServerHandleSubmit = async (e) => {
    ...
    if (imageUrl) {
      ...
      await dispatch(
        serverActions.uploadServerImageThunk(newServer.id, formData)
      );
    ...
    setType(null);
    return history.push(
      `/servers/${newServer.id}/channels/${newServer.firstChannel.id}`
    );
  };
  ...
  return (
    <form
      ...
      onSubmit={createServerHandleSubmit}
    >
      <div className="c-server-top-wrap">
        ...
        <div className="c-server-image-outer-wrapper">
          <div
            className="c-server-image-wrapper"
            style={{ backgroundColor: imageUrl ? "rgb(71,82,196)" : "" }}
          >
            {imageUrl ? (
              <FaThumbsUp className="c-server-thumb-icon" />
            ) : (
        ...
        <label className="c-server-name-label">
          SERVER NAME *
          <input
        ...
        <p className="c-server-agreement">
          By creating a server, you agree to Accord's
          <span className="c-server-guidelines"> Community Guidelines</span>
        </p>
      </div>
      <div className="c-server-bottom-wrap">
        <p className="c-server-back" onClick={closeModal}>
          Back
        </p>
        <button
          id={serverName.length < 1 ? "c-server-button-disabled" : ""}
          disabled={serverName.length < 1}
          className="c-server-create-button"
          type="submit"
        >
          Create
        </button>
